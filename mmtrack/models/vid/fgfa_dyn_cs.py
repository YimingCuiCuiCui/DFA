...
class FGFADynCS(BaseVideoDetector):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(FGFADynCS, self).__init__(init_cfg)
        ...
        self.motion_est_loss = nn.MSELoss()
    ...
    def forward_train(self,
        ...
        iou = ops.box_iou(gt_bboxes[0], ref_gt_bboxes[0][:2, 1:])

        if iou.nelement() == 0:
            iou = torch.FloatTensor([1]).cuda()
        else:
            iou = torch.max(iou)

        ref_len = ref_img.shape[1]


        if iou.detach().cpu().numpy() > 0.8:
            ref_len = min(2, ref_img.shape[1])
        elif iou.detach().cpu().numpy() > 0.6:
            ref_len = min(3, ref_img.shape[1])
        ...
        all_imgs = torch.cat((img, ref_img[0][:ref_len, :, :, :]), dim=0)
        all_x = self.detector.extract_feat(all_imgs)
        x = []

        nearest_imgs = torch.cat((img, ref_img[0][:2, :, :, :]), dim=0)
        nearest_x = self.detector.extract_feat(nearest_imgs)

        pooled_feats = self.avg_pool(nearest_x[0])
        pooled_feats = pooled_feats.view(pooled_feats.size(0), -1)
        pred_iou = torch.max(torch.sigmoid(self.fc_reg(pooled_feats)))

        motion_loss = self.motion_est_loss(pred_iou, iou)
        ...
        for i in range(len(all_x)):
            ...
            ref_x_list.append(ref_x_single)
        ...
        # print('Step 1, ', len(ref_x_list), ref_x_list[0].shape)
        ref_idx = 0
        curr_ref_x_list = [ref_x_list[0][ref_idx, :, :, :].unsqueeze(0)]
        # Two stage detector
        if hasattr(self.detector, 'roi_head'):
            # RPN forward and loss
            if self.detector.with_rpn:
                proposal_cfg = self.detector.train_cfg.get(
                    'rpn_proposal', self.detector.test_cfg.rpn)
                ref_rpn_losses, ref_proposal_list = \
                    self.detector.rpn_head.forward_train(
                        curr_ref_x_list,
                        img_metas,
                        gt_bboxes,
                        ...
                        gt_bboxes_ignore=gt_bboxes_ignore,
                        proposal_cfg=proposal_cfg)
                for kk in ref_rpn_losses.keys():
                    losses['ref_img_' + str(ref_idx) + "_" + kk] = ref_rpn_losses[kk]
            else:
                ref_proposal_list = proposals

            ref_roi_losses = self.detector.roi_head.forward_train(
                curr_ref_x_list, img_metas, ref_proposal_list, gt_bboxes, gt_labels,
                gt_bboxes_ignore, gt_masks, **kwargs)
            for kk in ref_roi_losses.keys():
                losses['ref_img_' + str(ref_idx) + "_" + kk] = ref_roi_losses[kk]
            # print(ref_roi_losses, ref_rpn_losses)
        # Single stage detector
        elif hasattr(self.detector, 'bbox_head'):
            ref_bbox_losses = self.detector.bbox_head.forward_train(
                curr_ref_x_list, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore)
            for kk in ref_bbox_losses.keys():
                losses['ref_img_' + str(ref_idx) + "_" + kk] = ref_bbox_losses[kk]
        else:
            raise TypeError('detector must has roi_head or bbox_head.')
        ...
        losses['motion_est_loss'] = motion_loss
        return losses
    ...
    def extract_feats(self, img, img_metas, ref_img, ref_img_metas):
        ...
        ref_len = self.memo.img.shape[0]
        rmv_len = 0

        if pred_iou.detach().cpu().numpy() > 0.75:
            ref_len = 11
            rmv_len = self.memo.img.shape[0] // 2 - ref_len // 2
        elif pred_iou.detach().cpu().numpy() > 0.5:
            ref_len = 21
            rmv_len = self.memo.img.shape[0] // 2 - ref_len // 2
        ...
        if rmv_len == 0:
            ...
            flow_imgs = torch.cat(
                (img.repeat(ref_len, 1, 1, 1), self.memo.img[rmv_len: -rmv_len, :, :, :]),
                dim=1)
        flows = self.motion(flow_imgs, img_metas)

        # print("          ", pred_iou, ref_img.shape, len(ref_img), self.memo.img.shape, len(x),
        #       self.memo.feats[0].shape, flows.shape, flow_imgs.shape)
        ...
        for i in range(len(x)):
            if rmv_len == 0:
                ...
                agg_x_single = flow_warp_feats(self.memo.feats[i][rmv_len: -rmv_len, :, :, :], flows)

            if frame_stride < 1:
                agg_x_single = torch.cat((x[i], agg_x_single), dim=0)
            else:
                if num_left_ref_imgs > agg_x_single.shape[0]:
                    agg_x_single[ref_len // 2] = x[i]
                else:
