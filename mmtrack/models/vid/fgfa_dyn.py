...
class FGFADyn(BaseVideoDetector):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(FGFADyn, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        if iou.detach().cpu().numpy() > 0.9:
            ref_len = min(3, ref_img.shape[1])
        elif iou.detach().cpu().numpy() > 0.8:
            ref_len = min(6, ref_img.shape[1])

        # print(iou, ref_img[:, 0].shape, ref_img.shape, ref_len, ref_img[0].shape)
        ...
        x = []
        # print(len(all_x), all_imgs.shape, all_x[0].shape, flows.shape)
        ...
        # print(pooled_feats.shape, pred_iou, iou, motion_loss)
    ...
    def extract_feats(self, img, img_metas, ref_img, ref_img_metas):
        ...
        if pred_iou.detach().cpu().numpy() > 0.95:
            ref_len = 9
            rmv_len = self.memo.img.shape[0] // 2 - ref_len // 2
        elif pred_iou.detach().cpu().numpy() > 0.9:
            ref_len = 13
            rmv_len = self.memo.img.shape[0] // 2 - ref_len // 2
